...
export default function RegisterMacroModal({ open, onClose, onDone, editEntry = null }) {
  ...
  const inputCls =
    "w-full rounded-lg bg-slate-800 border border-slate-700 px-3 py-2 text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 bg-black/60 overflow-y-auto">
      <div className="w-full max-w-2xl my-8 rounded-2xl bg-slate-900 border border-slate-700 shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-5">
          <div className="grid grid-cols-3 gap-4">
            <label className="block">
              <span className="text-sm text-slate-300 mb-1 block">아이디</span>
              <input
            ...
            <label className="block">
              <span className="text-sm text-slate-300 mb-1 block">비밀번호</span>
              <input
                className={inputCls}
                type="password"
                value={password}
            ...
            <label className="block">
              <span className="text-sm text-slate-300 mb-1 block">페이퍼 모드</span>
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-800 sticky bottom-0 bg-slate-900 rounded-b-2xl">
          <button onClick={onClose} disabled={busy} className="rounded-lg bg-slate-700 hover:bg-slate-600 px-5 py-2.5 font-semibold disabled:opacity-40">
            취소
          </button>
          <button onClick={save} disabled={busy || !!valErr} className="rounded-lg bg-indigo-600 hover:bg-indigo-500 px-5 py-2.5 font-semibold disabled:opacity-40">
