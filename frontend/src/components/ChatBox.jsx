...
export default function ChatBox() {
  ...
  return (
    <div className="mt-10 rounded-2xl bg-slate-900 border border-slate-800 p-5">
      ...
      <div ref={listRef} className="h-64 overflow-y-auto rounded-xl border border-slate-800 bg-slate-950/40 p-3 space-y-1.5">
        ...
        {items.map((m) => (
          <div key={m.id} className="text-sm">
            ...
            <span className="font-semibold text-indigo-300 mr-1.5">{m.username}</span>
      ...
      <form onSubmit={send} className="mt-3 flex gap-2">
        <input
          ...
          maxLength={24}
          placeholder="아이디"
          className="w-28 rounded-lg bg-slate-800 border border-slate-700 px-3 py-2 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
        />
        <input
          value={text}
          ...
          className="flex-1 rounded-lg bg-slate-800 border border-slate-700 px-3 py-2 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
        />
        <button type="submit" disabled={busy} className="rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 px-4 py-2 text-sm font-semibold">
          전송
        </button>
      </form>
